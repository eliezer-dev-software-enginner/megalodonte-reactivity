...
public class ListState<E> implements ReadableState<List<E>> {
    ...
    /**
     * Subscribes to list changes WITHOUT firing immediately with the current value.
     * Use this when you only care about future changes, not the current state.
     * Contrast with {@link #subscribe(Consumer)}, which fires immediately on registration.
     *
     * @param listener to be notified of list changes
     */
    ...
    /**
     * Forces notification of all listeners with the current list value,
     * without replacing the list reference.
     * <p>
     * Useful when items inside the list are mutated in-place and you need
     * to propagate the change reactively.
     * </p>
     */
    public void refresh() {
        for (var listener : List.copyOf(listeners)) {
